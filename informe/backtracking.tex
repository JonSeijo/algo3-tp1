...
Explicar por qué es $O(n*3^{n})$
Revisar bien pero creo que si, puedo demostrarlo haciendo la cuenta

en cada nivel son $3^{k}$ nodos, hay $n-1$ niveles y una hoja cuesta $n$.
Entonces queda la sumatoria desde 0 hasta $n-1$ de $3^{i} * n$  $=$  $O(n*3^{n})$
